...
WAF_SIGNATURES = {
    ...
    'AWS WAF': {
        ...
        'body': [
            r'<html><head><title>ERROR: The request could not be satisfied',
            r'Request blocked',
    ...
    'Alibaba Cloud WAF': {
        ...
        'body': [
            r'Blocked by Alibaba Cloud Security',
            r'errors\.aliyun\.com',
        ],
    ...
    'Armor Defense': {
        ...
        'body': [
            r'Armor',
            r'blocked by website protection',
        ],
        'status_codes': [403],
    },
    'WebKnight': {
...
def print_menu():
    ...
    │  {Fore.YELLOW}[8]{Fore.WHITE} ◆  WAF Detection                                              {Fore.CYAN}│
...
def test_url_full(url, timeout=10):
    """Test URL and return full response info including headers."""
    try:
        headers = {
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36',
            'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8',
...
def detect_waf(response_data):
    ...
    resp_headers = {k.lower(): v for k, v in response_data.get('headers', {}).items()}
    resp_body = response_data.get('text', '')
    ...
    for waf_name, signatures in WAF_SIGNATURES.items():
        ...
        for header_name, header_pattern in signatures.get('headers', []):
            header_value = resp_headers.get(header_name.lower(), '')
        ...
        for body_pattern in signatures.get('body', []):
            if re.search(body_pattern, resp_body, re.IGNORECASE):
                confidence += 25
                matches.append(f"Body pattern match")
                break
        
        if resp_status in signatures.get('status_codes', []):
            confidence += 15
    ...
    if detected_wafs:
        return detected_wafs[0]['name'], detected_wafs[0]['confidence']
    
    return None, 0


def print_waf_detected(url, waf_name, confidence):
    """Print WAF detection result."""
    color = Fore.RED if confidence >= 70 else Fore.YELLOW
    print(f"""
    {color}╔{'═'*66}╗
    ║{Back.RED}{Fore.WHITE}  WAF DETECTED  {Style.RESET_ALL}{color}║ Web Application Firewall Found!          ║
    ╠{'═'*66}╣{Style.RESET_ALL}
    {color}║{Fore.YELLOW} URL:{Style.RESET_ALL} {url[:58]}{'...' if len(url) > 58 else ''}{' ' * max(0, 58 - len(url[:58]))} {color}║
    {color}║{Fore.YELLOW} WAF:{Style.RESET_ALL} {waf_name}{' ' * (54 - len(waf_name))} {color}║
    {color}║{Fore.YELLOW} Confidence:{Style.RESET_ALL} {confidence}%{' ' * (49 - len(str(confidence)))} {color}║
    {color}╚{'═'*66}╝{Style.RESET_ALL}
""")


def check_waf(url, timeout=10, silent=False):
    """Check if URL is protected by a WAF."""
    test_payloads = [
    ...
    response_data = test_url_full(url, timeout)
    if not response_data:
        return None, 0
    
    waf_name, confidence = detect_waf(response_data)
    
    if not waf_name or confidence < 50:
        for payload in test_payloads:
            test_url_payload = url.split('?')[0] + payload
            response_data = test_url_full(test_url_payload, timeout)
            if response_data:
                waf_name_new, confidence_new = detect_waf(response_data)
                if confidence_new > confidence:
                    waf_name = waf_name_new
                    confidence = confidence_new
                if confidence >= 70:
                    break
    
    if waf_name and not silent:
        with thread_lock:
            print_waf_detected(url, waf_name, confidence)
...
def menu_waf_detection():
    """Menu for WAF Detection."""
    clear_screen()
    print_banner()
    
    print(f"""
    {Fore.CYAN}╔════════════════════════════════════════════════════════════════════╗
    ║  {Fore.YELLOW}◆  WAF DETECTION{Fore.CYAN}                                                  ║
    ╠════════════════════════════════════════════════════════════════════╣
    ║  {Fore.WHITE}Mendeteksi Web Application Firewall pada target URL.{Fore.CYAN}               ║
    ║  {Fore.WHITE}Mendukung {Fore.GREEN}{len(WAF_SIGNATURES)}{Fore.WHITE} WAF termasuk:{Fore.CYAN}                                     ║
    ║  {Fore.YELLOW}Cloudflare, AWS WAF, Akamai, Imperva, Sucuri, ModSecurity,{Fore.CYAN}        ║
    ║  {Fore.YELLOW}F5 BIG-IP, Fortinet, Barracuda, Wordfence, dll.{Fore.CYAN}                   ║
    ╠════════════════════════════════════════════════════════════════════╣
    ║  {Fore.YELLOW}[1]{Fore.WHITE} Check Single URL                                            {Fore.CYAN}║
    ║  {Fore.YELLOW}[2]{Fore.WHITE} Check Multiple URLs dari File                               {Fore.CYAN}║
    ║  {Fore.YELLOW}[3]{Fore.WHITE} Lihat Daftar WAF yang Didukung                              {Fore.CYAN}║
    ║  {Fore.YELLOW}[0]{Fore.WHITE} Kembali                                                     {Fore.CYAN}║
    ╚════════════════════════════════════════════════════════════════════╝
{Style.RESET_ALL}""")
    ...
    if choice == '1':
        url = input(f"    {Fore.YELLOW}➤{Style.RESET_ALL} Masukkan URL: ").strip()
        if url:
            if not url.startswith(('http://', 'https://')):
                url = 'http://' + url
            print()
            print_info(f"Checking WAF for: {url}")
            waf_name, confidence = check_waf(url, timeout=settings['timeout'])
            if not waf_name:
                print_success(f"Tidak terdeteksi WAF pada {url}")
        else:
            print_error("URL tidak boleh kosong!")
    
    elif choice == '2':
        filepath = input(f"    {Fore.YELLOW}➤{Style.RESET_ALL} Path file URL: ").strip()
        if os.path.exists(filepath):
            with open(filepath, 'r') as f:
                urls = [line.strip() for line in f if line.strip()]
            
            print_info(f"Checking {len(urls)} URLs...")
            print()
            ...
            for url in urls:
                if not url.startswith(('http://', 'https://')):
                    url = 'http://' + url
                waf_name, confidence = check_waf(url, timeout=settings['timeout'], silent=True)
                if waf_name:
                    if waf_name not in waf_results:
                        waf_results[waf_name] = []
                    waf_results[waf_name].append((url, confidence))
            
            print(f"""
    {Fore.CYAN}╔{'═'*66}╗
    ║  {Fore.YELLOW}WAF DETECTION RESULTS{Fore.CYAN}{' '*43}║
    ╠{'═'*66}╣""")
            
            if waf_results:
                for waf_name, urls_list in waf_results.items():
                    print(f"    {Fore.CYAN}║  {Fore.GREEN}{waf_name}:{Style.RESET_ALL} {len(urls_list)} URL(s){' '*(49-len(waf_name)-len(str(len(urls_list))))}{Fore.CYAN}║")
                    for url, conf in urls_list[:3]:
                        url_display = url[:50] + '...' if len(url) > 50 else url
                        print(f"    {Fore.CYAN}║    {Fore.WHITE}- {url_display}{' '*(57-len(url_display))}{Fore.CYAN}║")
            else:
                print(f"    {Fore.CYAN}║  {Fore.YELLOW}Tidak ada WAF terdeteksi pada URL yang dicek.{Fore.CYAN}{' '*16}║")
        ...
        print(f"""
    {Fore.CYAN}╔{'═'*66}╗
    ║  {Fore.YELLOW}SUPPORTED WAFs ({len(WAF_SIGNATURES)} total){Fore.CYAN}{' '*(44-len(str(len(WAF_SIGNATURES))))}║
    ╠{'═'*66}╣""")
        
        waf_list = list(WAF_SIGNATURES.keys())
        for i in range(0, len(waf_list), 2):
            waf1 = waf_list[i]
            waf2 = waf_list[i+1] if i+1 < len(waf_list) else ''
            line = f"    {Fore.CYAN}║  {Fore.GREEN}• {waf1:<28}{Style.RESET_ALL}"
            if waf2:
                line += f"  {Fore.GREEN}• {waf2:<28}{Style.RESET_ALL}"
            else:
                line += f"  {' '*31}"
            print(line + f"{Fore.CYAN}║")
